...
/** volumetrics contract — Froxels (dust/haze) plugs in when it lands */
export interface VolumetricsLike {
  apply(col: NV3, dist: NF, uv: NV2): NV3;
}
...
export class PostStack {
  ...
  constructor(
    ...
    froxels: VolumetricsLike | null = null,
  ) {
    ...
    // --- heat shimmer (brief §2/§3.14): near-surface refraction at high sun.
    // Distant low-elevation rays over sun-baked ground pick up an animated
    // vertical wobble; the beauty buffer is sampled at the shimmered uv.
    // uShimmer = CPU sun-altitude gate (0 except midday hours); worldTime
    // drives the animation and freezes under ?freeze for determinism.
    const uShimmer = runiform(0);
    const uWorldTime = runiform(0);
    this.setShimmer = (k): void => {
      uShimmer.value = k;
    };
    this.tickTime = (t): void => {
      uWorldTime.value = t;
    };
    const shimmeredUv = Fn((): NV2 => {
      const d0 = depthTex.x;
      const isSky0 = d0.lessThanEqual(1e-7).or(d0.greaterThanEqual(0.9999999));
      const viewDirV0 = getViewPosition(screenUV, float(0.5), uProjInv).normalize();
      const dirW0 = uCamWorld.mul(vec4(viewDirV0, 0)).xyz;
      const dist0 = getViewPosition(screenUV, d0, uProjInv).length();
      // strongest for grazing rays 40–800 m out; none on sky or near field
      const grazing = smoothstep(0.25, 0.04, dirW0.y.abs());
      const band = smoothstep(40, 120, dist0).mul(smoothstep(1400, 500, dist0));
      const k = float(uShimmer)
        .mul(grazing)
        .mul(band)
        .mul(isSky0.select(float(0), float(1)));
      const t = float(uWorldTime);
      const wob = hash12(
        screenUV.mul(vec2(230.7, 71.3)).add(t.mul(vec2(0.9, 1.7)).fract()),
      )
        .sub(0.5)
        .mul(2);
      const wob2 = hash12(
        screenUV.mul(vec2(97.1, 189.3)).add(t.mul(vec2(1.3, 0.7)).fract()),
      )
        .sub(0.5)
        .mul(2);
      return screenUV.add(vec2(wob2.mul(0.4), wob).mul(k).mul(0.0028));
    })();

    // --- aerial perspective from depth -----------------------------------------
    const aerialNode = Fn((): NV3 => {
      const d = depthTex.x.toVar();
      const col = (texture(beauty.value, shimmeredUv) as unknown as NV4).rgb.toVar();
      ...
      // ASL altitude: world Y datum is 60 m ASL (CANON_DIMENSIONS)
      const camAltKm = camPosW.y.add(60).div(1000).max(0.005);
      // sky = cleared depth; tolerate either depth convention (0 or 1 at far)
      const isSky = d.lessThanEqual(1e-7).or(d.greaterThanEqual(0.9999999));
      // froxel volumetrics first (local dust plumes ≤ ~480 m), the km-scale
      // Hillaire haze integrates on top of the fogged radiance
      if (froxels) {
        const fogDist = isSky.select(float(1e5), dist);
        col.assign(froxels.apply(col, fogDist, screenUV));
      }
      // desert vs river-valley haze profile by the fragment's world x
      // (the Nile corridor lies east of x ≈ +1500 in the world frame)
      const fragW = camPosW.add(dirW.mul(dist));
      const valleyK = smoothstep(900, 1900, fragW.x).mul(0.85);
      const hazed = atmosphere.aerial(col, dirW, camAltKm, distKm, valleyK);
  ...
  private setShimmer: (k: number) => void = () => undefined;
  private tickTime: (t: number) => void = () => undefined;
  ...
  setTimeOfDay(tod: number): void {
    ...
    // heat shimmer only under a high sun (canon: midday heat shimmer)
    const noonness = Math.max(0, 1 - Math.abs(tod - 12.5) / 3.2);
    this.setShimmer(noonness * noonness);
  }
  ...
  render(): void {
    ...
    this.tickTime(this.engineRef.worldTime);
    this.post.render();
  }
}
