 ...
 * `window.__akhet.stats` for the verification harness.
 * Core pattern ported from PROJECT LAAS (MIT) — see LICENSE-THIRD-PARTY.
 */
...
export class Engine {
  ...
  readonly params: AkhetParams;
  readonly hooks: AkhetHooks;
    ...
    if (device) {
      ...
      device.onuncapturederror = (e: GPUUncapturedErrorEvent): void => {
        if (reported++ < 8) {
          ...
          console.error('[akhet] WebGPU uncaptured error:', e.error.message);
        }
      };
    }
  ...
  private frame(timeMs: number): void {
    ...
    // CPU attribution: update = app-side per-frame work, submit = three
    // render+encode (backpressure shows as the gap to frameMs)
  ...
  private collectStats(rawDt: number): void {
    ...
    // resolve EVERY frame: the 2048-query pool only resets its write index on
    // resolve — a slower cadence overflows it and kills per-pass attribution
