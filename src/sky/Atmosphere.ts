...
// rayleigh standard; mie at a DRY-desert level (Egyptian air is clear with
// suspended fine dust — LAAS's humid 2.4× boost buried the plateau in gray
// by 2 km; horizontal visibility here should be ~25-40 km)
const BETA_R: [number, number, number] = [5.802e-3, 13.558e-3, 33.1e-3];
const BETA_M_S = 4.8e-3;
const BETA_M_E = 5.4e-3;
...
export class Atmosphere {
   ...
   * DUST layer (dry Egyptian boundary layer — Pillar D: haze attenuates,
   * never hides). `valleyK` 0..1 selects the moister river-valley profile
   * over the open-desert one (brief §2: two distinct haze profiles).
   * Altitudes are ASL km (world Y + 60 m, handled by the caller).
   */
  aerial(color: NV3, viewDir: NV3, camAltKm: NF, distKm: NF, valleyK?: NF): NV3 {
    ...
    // dust layer: density k·exp(−h/Hf); exact path integral. Desert profile
    // is thin suspended dust; the valley adds low-hugging moisture.
    const vk = valleyK ?? float(0);
    const DUST_HF = 0.55; // scale height, km
    const dustK = mix(float(0.038), float(0.065), vk); // extinction /km at 0 ASL
    const y0 = camAltKm.div(DUST_HF);
    const y1 = fragAlt.div(DUST_HF);
    const dy = y1.sub(y0);
    ...
    const tauF = integ.mul(dustK).max(0);
    // dust extinguishes blue slightly more → distant light warms while the
    // scattered veil stays warm-gray (canon: fine airborne dust, no gray murk)
    const dustSpectrum = vec3(0.88, 1.0, 1.18);

    const T = vexp3(tauR.add(tauM).add(dustSpectrum.mul(tauF)).negate());
    const sky = this.skyColor(viewDir);
    // in-scatter: sky radiance, warmed toward sand tone by the dust share
    const dustShare = tauF.div(tauF.add(tauM).add(0.35).max(1e-3)).clamp(0, 1);
    const src = mix(sky, sky.mul(vec3(1.12, 1.0, 0.82)), dustShare.mul(0.7));
    return color.mul(T).add(src.mul(vec3(1).sub(T)));
  }
